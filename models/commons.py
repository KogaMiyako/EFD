...
class SENetBlock(nn.Module):
    def __init__(self, dim, dim_out, *, time_emb_dim=0, groups=8):
        ...
        if time_emb_dim > 0:
            self.mlp = nn.Sequential(
                nn.Linear(time_emb_dim, dim_out),
                nn.Sigmoid()  # Apply sigmoid to match SE module's gating mechanism
            )
        self.block1 = Block(dim, dim_out, groups=groups)
        self.block2 = Block(dim_out, dim_out, groups=groups)
        self.se = SqueezeExciteBlock(dim_out)  # Add SE module
        self.res_conv = nn.Conv2d(dim, dim_out, 1) if dim != dim_out else nn.Identity()

    def forward(self, x, time_emb=None, cond=None):
        h = self.block1(x)
        if time_emb is not None:
            mlp_output = self.mlp(time_emb)[:, :, None, None]
            h = h * mlp_output  # Apply SE mechanism using MLP output
        if cond is not None:
            h += cond
        h = self.block2(h)
        h = self.se(h)  # Apply SE mechanism on the output
